...
class MessageController extends Controller
{
     ...
     */
    public function deleteMessage(Request $request,$id, UserInterface $user)
    {
        $userId = $user->getId();
        
        $repo=$this->getDoctrine()->getRepository(Message::class);
        $message=$repo->find($id);
        if ($userId==$message->getUser()->getId()) {
            $em = $this->getDoctrine()->getManager();
            $em->remove($message);
            $em->flush();
            return $this ->redirectToRoute('messages'); 
     ...
     */
    public function likeMessage(Request $request,$id, UserInterface $user)
    {
        $userId = $user->getId();
